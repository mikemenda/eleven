...
const legendStats = (player) => {
  ...
  const cg      = apps > 0 ? ((goals + assists) / apps).toFixed(2) : '—'
  return [
    { val: apps, label: 'Apps' },
    { val: ga,   label: 'G+A'  },
    { val: cg,   label: 'C/G'  },
...
const ClubHero = ({ club, game, seasons }) => {
  ...
  const record = wins + draws + losses > 0 ? `${wins}W–${draws}D–${losses}L` : null
    ...
    <div className={styles.legacyStrip}>
      {winRate !== null && (
        <div className={styles.legacyStat}>
          <div className={styles.legacyVal}>
            {winRate}<span className={styles.legacyUnit}>%</span>
          </div>
          <div className={styles.legacyLabel}>Win Rate</div>
        </div>
      )}
      {winRate !== null && gpg !== null && <div className={styles.legacyDivider} />}
      {gpg !== null && (
        <div className={styles.legacyStat}>
          <div className={styles.legacyVal}>{gpg}</div>
          <div className={styles.legacyLabel}>Goals / Game</div>
        </div>
      )}
      {(winRate !== null || gpg !== null) && trophyCount > 0 && <div className={styles.legacyDivider} />}
      {trophyCount > 0 && (
        <div className={styles.legacyStat}>
          <div className={styles.legacyVal}>{trophyCount}</div>
          <div className={styles.legacyLabel}>Trophies</div>
        </div>
      )}
    </div>
  )
      ...
      {accomplishments.length > 0 && (
        <div className={styles.peakAccomplishments}>
          {accomplishments.map((a, i) => (
            <span key={i} className={styles.peakChip}>{a}</span>
          ))}
        </div>
      )}
    </Link>
        ...
        {ordered.map(s => {
          ...
          return (
            <Link key={s.id} to={`/seasons/${s.id}`} className={styles.arcRow}>
              <span className={`${styles.arcSeasonLabel} ${isPeak ? styles.arcSeasonLabelPeak : ''}`}>
                {s.label}
              </span>
